...
const document = {
  getElementById: el,
  querySelector: selector => ({checked: selector.includes('"good"') || selector.includes('"asr_issue"')}),
  addEventListener: () => {}
};
const localStorage = {
...
if (first.annotations[0].labels.join(",") !== "good,asr_issue") throw Error("DOM state was not exported");
