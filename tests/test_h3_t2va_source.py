...
from r2v_data_v2.h3 import t2va_source as source_module
...
def test_case_manifest_uses_uid_index_and_only_parses_selected_rows(
    tmp_path, monkeypatch
):
    source = tmp_path / "source"
    source.mkdir()
    manifest = fixtures.shot_manifest(source)
    template = json.loads(manifest.read_text().splitlines()[0])
    rows = []
    for index in range(1000):
        video = source / f"movie_{index + 1}.mp4"
        rows.append({**template, "video_path": str(video), "shot_index": index + 1})
    for index in (1, 3):
        Path(rows[index]["video_path"]).write_bytes(b"selected video")
    manifest.write_text("".join(json.dumps(row) + "\n" for row in rows))
    selected_uids = [
        parse_clip_identity(rows[index]["video_path"]).clip_uid for index in (3, 1)
    ]
    case = tmp_path / "cases.json"
    case.write_text(json.dumps({"clip_uids": selected_uids}))
    cache = tmp_path / "cache"
    options = {"case_manifest": case, "shot_index_root": cache}
    original_parse = JeaVideoMotionAdapter.parse
    parsed = []

    def parse(self, raw, *, source_index):
        parsed.append(source_index)
        return original_parse(self, raw, source_index=source_index)

    monkeypatch.setattr(JeaVideoMotionAdapter, "parse", parse)
    first = select_t2va_shots(manifest, **options)
    assert parsed == [3, 1]
    assert [shot.source_index for shot in first.shots] == [3, 1]
    assert [shot.clip_uid for shot in first.shots] == selected_uids
    assert first.shot_manifest_sha256 == t2va.sha256_file(manifest)
    assert first.valid_row_count is None
    parsed.clear()

    # A warm lookup must not iterate the source or rehash the large index files.
    original_open = Path.open
    original_hash = shot_index.sha256_file
    reads = []
    ...
    class SeekOnly:
        ...
        def readline(self):
            reads.append(self.handle.tell())
            return self.handle.readline()

    def open_file(path, *args, **kwargs):
        handle = original_open(path, *args, **kwargs)
        return SeekOnly(handle) if path == manifest else handle

    hashed = []

    def hash_file(path):
        assert path not in {manifest, *cache.glob("*/offsets.bin")}
        hashed.append(path)
        return original_hash(path)

    with monkeypatch.context() as patch:
        patch.setattr(Path, "open", open_file)
        patch.setattr(shot_index, "sha256_file", hash_file)
        patch.setattr(source_module, "sha256_file", hash_file)
        patch.setattr(
            shot_index,
            "_uid_index_writer",
            lambda *args, **kwargs: pytest.fail("warm UID index rebuilt"),
        )
        again = select_t2va_shots(manifest, **options)
    assert again == first
    assert parsed == [3, 1]
    assert len(reads) == 2
    assert hashed == [Path(rows[index]["video_path"]) for index in (3, 1)]


def test_case_manifest_uid_index_rejects_missing_and_duplicate_uids(tmp_path):
    manifest = fixtures.shot_manifest(tmp_path)
    uid = parse_clip_identity(
        json.loads(manifest.read_text().splitlines()[0])["video_path"]
    ).clip_uid
    case = tmp_path / "cases.json"
    options = {
        "case_manifest": case,
        "shot_index_root": tmp_path.parent / f"{tmp_path.name}-cache",
    }
    case.write_text(json.dumps({"clip_uids": ["missing"]}))
    with pytest.raises(ValueError, match="selected clips must be unique members"):
        select_t2va_shots(manifest, **options)
    case.write_text(json.dumps({"clip_uids": [uid, uid]}))
    with pytest.raises(ValueError, match="selected clips must be unique members"):
        select_t2va_shots(manifest, **options)


def test_case_manifest_reuses_random_offset_index(tmp_path):
    manifest = fixtures.shot_manifest(tmp_path)
    cache = tmp_path.parent / f"{tmp_path.name}-cache"
    select_t2va_shots(manifest, sample_size=1, sample_seed=7, shot_index_root=cache)
    uid = parse_clip_identity(
        json.loads(manifest.read_text().splitlines()[1])["video_path"]
    ).clip_uid
    case = tmp_path / "cases.json"
    case.write_text(json.dumps({"clip_uids": [uid]}))
    result = select_t2va_shots(manifest, case_manifest=case, shot_index_root=cache)
    assert [shot.clip_uid for shot in result.shots] == [uid]
    assert len(list(cache.glob("*/metadata.json"))) == 1
    assert len(list(cache.glob("*/uids.bin"))) == 1


def test_case_manifest_uid_index_detects_duplicate_source_identity(tmp_path):
    manifest = fixtures.shot_manifest(tmp_path)
    lines = manifest.read_text().splitlines()
    manifest.write_text("\n".join([lines[0], lines[0]]) + "\n")
    uid = parse_clip_identity(json.loads(lines[0])["video_path"]).clip_uid
    case = tmp_path / "cases.json"
    case.write_text(json.dumps({"clip_uids": [uid]}))
    with pytest.raises(ValueError, match="duplicate JEA shot clip identity"):
        select_t2va_shots(
            manifest,
            case_manifest=case,
            shot_index_root=tmp_path.parent / f"{tmp_path.name}-cache",
