...
def _case(tmp_path, monkeypatch, original, response, speakers=("S1",), transport="sglang"):
    ...
    if transport == "xiaomi":
        ...
        def create(**request):
            ...
            if request["response_format"] == {"type": "json_object"}:
                completions.requests.append(request)
                index = (mb.VISUAL_SYSTEM_PROMPT, mb.SYSTEM_PROMPT, mb.SPEAKER_PROFILE_SYSTEM_PROMPT, mb.AUDIO_FINALIZE_SYSTEM_PROMPT).index(request["messages"][0]["content"])
                return _Completions([(split_annotation(raw)[index], 8)]).create(**request)
    ...
    assert row["av_model_call_count"] == 2 and row["audio_model_call_count"] == 1
...
def test_polish_v4_prompt_makes_aligned_speaker_projection_unambiguous():
    ...
    assert mb.MIMO25_BACKEND_VERSION == "r2v.h3.mimo25_backend.59"
...
def test_polish_summary_audit_and_qa_keep_av_raw_separate(tmp_path, monkeypatch):
    ...
    assert (summary.audio_model_call_count, summary.av_model_call_count, summary.text_model_call_count, summary.model_call_count) == (3, 6, 3, 15)
