...
def test_old_prepared_and_done_survive_group_change(tmp_path, monkeypatch):
    ...
    assert old["identity_details"]["contract"] == "text_two_person_pdd_fsdp2_pair_v12"
    legacy_details = {**old["identity_details"],"contract":"text_two_person_pdd_fsdp2_pair_v11"}
    legacy_identity = hashlib.sha256(json.dumps(legacy_details,sort_keys=True).encode()).hexdigest()
    legacy_root = tmp_path/"legacy-v11"
