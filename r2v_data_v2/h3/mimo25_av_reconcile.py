...
class MimoInventory(BindingEvidenceSource):
    schema_version: Literal["r2v.h3.mimo25_inventory.4", "r2v.h3.mimo25_inventory.5", "r2v.h3.mimo25_inventory.6"] = MIMO25_INVENTORY_VERSION
    ...
    def validate_inventory(self) -> MimoInventory:
        if (not self.schema_version.endswith(".4")) != (self.binding_evidence_mode == "none"):
...
def load_mimo25_reference_sources(
    *, visual_production_root: Path, visual_runs_root: Path,
    ...
    if (len(canonical_by_clip) != len(canonical)
        ...
        or set(canonical_by_clip) != set(visual_by_clip)):
    ...
    for clip in visual.canonical_clips:
        ...
        samples.append(FinalH3SampleV2(
            **clip.identity.model_dump(mode="python"),
            sample_id=f"{clip.sample.sample_id}/canonical", pair_id=f"canonical/{uid}",
            pair_type="canonical", target_video=audio.target_video_path,
            target_full_audio_path=audio.target_full_audio_path,
            target_full_audio_sha256=audio.target_full_audio_sha256,
            r2v_instruction=clip.sample.r2v_instruction,
            visual_references=[FinalVisualReference.from_visual(r) for r in clip.sample.references],
            subject_voices=[], speech_segments=[],
        ))
    return samples, canonical_by_clip
...
def build_mimo25_reference_inventory(
    *, visual_production_root: Path, visual_runs_root: Path,
    ...
    selected = case_manifest.clip_uids if case_manifest is not None else list(by_clip)
    if set(selected) - set(by_clip):
    ...
    for clip_uid in selected:
        ...
        jobs.append(_job({
            ...
            "segments": [], "source_h3_sample_ids": [representative.sample_id],
        }))
    return _inventory({
        "schema_version": "r2v.h3.mimo25_inventory.6", "binding_evidence_mode": "none",
        ...
        "source_h3_samples_sha256": _sha256_text("".join(s.model_dump_json() + "\n" for s in samples)),
