...
const jsonLineBreak={json.dumps(chr(10))};
...
function exportSelection(){{syncReviewState();const reviews=reviewRows();const selected=reviews.filter(row=>row.label==='background-rich').map(row=>row.target_clip_uid);if(selected.length<1){{alert('Select at least one background-rich clip.');return;}}const payload={{schema_version:'{BACKGROUND_AUDIO_SELECTION_VERSION}',source_scout_inventory_fingerprint:sourceScoutFingerprint,source_diarization_inventory_fingerprint:sourceDiarizationFingerprint,source_audio_evidence_fingerprint:sourceAudioFingerprint,selection_method:'manual_human_review_v1',selection_count:selected.length,selected_clip_ids:selected,reviews:reviews}};const blob=new Blob([JSON.stringify(payload,null,2)+jsonLineBreak],{{type:'application/json'}});const a=document.createElement('a');a.href=URL.createObjectURL(blob);a.download='background_audio_pilot_selection.json';a.click();URL.revokeObjectURL(a.href);}}
