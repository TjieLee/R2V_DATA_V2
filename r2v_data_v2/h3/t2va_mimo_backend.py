...
Also return one short internal summary of the target video at a compact semantic level. Summarize what is happening: the main visible subject or subjects, their salient action or interaction, the setting when useful, and important speech presentation when it matters. Choose a natural sentence structure from the actual semantic content rather than mechanically starting from any one description category. Camera scale, framing, angle, lighting, style, setting, subject, or action may be mentioned when useful, but none is required to come first. This summary is a downstream fact, not a public section: no reference labels, Sx tokens, dialogue markup, ASR words, task prefix, Audio reuse/reference wording or section headers. Do not change the detailed description to match this summary.
...
Write integrated_sequence as an ordered sequence of typed parts. A prose part contains kind=prose and text; a speech part contains kind=speech, segment_id and lead_in ONLY. This sequence becomes the final integrated multimodal description. Begin its first prose part with [Shot 1]. After [Shot 1], enter the observed scene naturally using whichever information is most salient in that particular video. The opening may foreground a subject, an action or interaction, the setting, an object or state, a spatial relationship, lighting or visual style, or camera/framing when that is genuinely informative; no category is required to come first. Preserve useful framing and camera facts, but do not treat them as a mandatory opening slot. Avoid generic meta-narration about the existence or beginning of the video, and avoid repeatedly defaulting to any single grammatical construction across clips. Let sentence structure follow the observed content rather than a fixed caption template. The first shot has no timestamp. Use later sequential [Shot N] markers only for real cuts/transitions, beginning each with At MM:SS.mmm, with strictly increasing cut times inside the supplied target duration. Describe camera motion, amplitude and speed when observable, or the static camera. Preserve readable on-screen text verbatim in double quotes.
