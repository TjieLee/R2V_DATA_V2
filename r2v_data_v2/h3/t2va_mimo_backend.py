...
Also return one short internal summary of the target video at a compact semantic level. Summarize what is happening: the main visible subject or subjects, their salient action or interaction, the setting when useful, and important speech presentation when it matters. Do not make shot scale, framing, camera angle, or camera vocabulary the default subject of the summary, and do not routinely begin with phrases such as "A close-up shot", "A medium shot", "A wide shot", or equivalent camera-first wording. Mention camera behavior only when it is genuinely central to the event. Vary the sentence structure according to the actual semantic content. This summary is a downstream fact, not a public section: no reference labels, Sx tokens, dialogue markup, ASR words, task prefix, Audio reuse/reference wording or section headers. Do not change the detailed description to match this summary.
...
Write integrated_sequence as an ordered sequence of typed parts. A prose part contains kind=prose and text; a speech part contains kind=speech, segment_id and lead_in ONLY. This sequence becomes the final integrated multimodal description. Begin its first prose part with [Shot 1]. Immediately after [Shot 1], write one concise sentence establishing the overall visual treatment of the clip—such as medium/style, lighting, color, contrast, or texture—using only what is actually observable. Keep that first sentence globally descriptive rather than making shot scale, camera angle, framing geometry, a subject action, dialogue, or chronology its grammatical focus. Do not force a decorative style label when the image is visually ordinary, and do not repeatedly default to one construction such as "Live-action, cinematic" or "The footage has...". This first sentence must stand on its own because an audio-conditioned downstream renderer may place it immediately before [Shot 1]. After that sentence, move directly into the concrete Shot 1 composition, subjects, environment, and action. Do not introduce the shot with generic meta-narration such as "The video opens", "The scene opens", "The clip begins", "We see", "The video shows", or equivalent boilerplate. Also do not systematically lead the shot-specific prose with camera geometry such as "A close-up shot frames...", "A medium shot shows...", "An over-the-shoulder shot frames...", or equivalent patterns. If a close-up, wide view, over-the-shoulder composition, camera angle, or other framing fact is genuinely observed, preserve that fact but place it naturally where it helps describe the composition rather than using it as a stock opener. Vary the grammatical entry point according to what is visually most informative. Examples of different two-step openings include "[Shot 1] Warm window light and subdued earth tones give the scene a naturalistic indoor look. Near a rain-streaked window, a young woman turns toward ...", "[Shot 1] Rendered in clean 2D animation, the image uses saturated colors and crisp outlines. A red bus pulls alongside ...", "[Shot 1] Cool, low-contrast color and flat fluorescent light create a restrained documentary texture. Two coworkers stand beside ...", "[Shot 1] Soft haze, amber practical lights, and muted blacks shape a subdued cinematic interior. Holding a folded letter at chest height, a man steps ...", and "[Shot 1] Blue-green caustic light defines a stylized underwater CG scene. A diver moves past ...". These are structural examples only, not templates, preferred phrasings, or a closed list of valid openings. The first shot has no timestamp. Use later sequential [Shot N] markers only for real cuts/transitions, beginning each with At MM:SS.mmm, with strictly increasing cut times inside the supplied target duration. Describe camera motion, amplitude and speed when observable, or the static camera. Preserve readable on-screen text verbatim in double quotes.
...
For every assignment also report acoustic waveform-ownership evidence: primary_speaker_ownership_resolved (whether the primary voice ownership is resolved), vocal_composition, and secondary_vocal_activity. Use the supplied RA2VA vocabulary: single_speaker, same_speaker_nonlexical, secondary_non_speech_vocalization, overlapping_secondary_speech, sequential_multi_speaker_speech, or uncertain. Multiple speakers, including sequential brief responses, must not be described as single_speaker. Secondary activity records whether it is present, its kind, and whether it belongs to the same speaker, a different speaker, or is uncertain; absent activity uses speaker_relation=none and kind=null. same_speaker_nonlexical means present non-speech activity attributable to the same primary speaker. These fields are eligibility evidence only: do not change Sx, speech presentation, boundaries or ASR because reuse might be unsafe. They do not belong in public prose and do not create entity binding.
